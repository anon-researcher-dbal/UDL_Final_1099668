"""Script for Experiment 2 (Deterministic vs Bayesian)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Experiment 2: Deterministic vs Bayesian (all acquisitions)"
    )
    ...
    parser.add_argument("--result_dir", type=str, default="exp2_results")
...
def main() -> None:
    ...
    args.determ = True  # run deterministic and Bayesian
    ...
    for name, curve in results.items():
        save_as_npy(data=curve, folder=args.result_dir, name=name)

    plot_and_save_results(
        results,
        args.result_dir,
        title="Deterministic vs Bayesian Active Learning (Experiment 2)",
        exp_type="mixed",
    )
